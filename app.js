...
/* Restores the old design's in-page ad placement (previously a sidebar slot)
   next to every tool's card, instead of only top/bottom of the whole page. */
function insertToolAdSlot(){
  const shell = document.querySelector('.tool-shell');
  if(!shell) return;
  const card = shell.querySelector('.tool-card');
  if(!card) return;
  const slot = document.createElement('div');
  slot.className = 'ad-slot rectangle';
  slot.style.margin = '22px auto 0';
  slot.setAttribute('data-i18n','ad_placeholder');
  slot.textContent = t('ad_placeholder');
  card.after(slot);
}
...
function stopwatchView(mode){ // mode: 'stopwatch' | 'splits'
  ...
  <div class="tool-shell" id="swContainer">
    ...
    <div class="tool-card">
      ...
      <div class="big-display" id="swTime" role="timer" aria-live="polite">00:00<small id="swMs">.000</small></div>
...
function countdownView(){
  ...
  <div class="tool-shell" id="cdContainer">
    ...
    <div class="tool-card">
      <button class="fs-exit" id="fsExit" style="display:none;">✕</button>
      <div class="big-display" id="cdTime" role="timer" aria-live="polite">01:00:00</div>
